...
export class DefaultTableComponent implements OnInit {
    ...
    @Output() onBlock = new EventEmitter<ModalValue>();
    @Output() onUnBlock = new EventEmitter<ModalValue>();
     ...
     */
    block(val: any) {
        this.onBlock.emit({ ref: this.blockRef, content: val });
    }
     ...
     */
    unBlock(val: any) {
        this.onUnBlock.emit({ ref: this.unBlockRef, content: val });
    }
